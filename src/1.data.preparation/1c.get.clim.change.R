...
#              (Apr–Sep), and derive a per-pixel warming trend raster (°C/year).
...
#### Download + cache annual growing-season mean per year ####

ann_paths <- setNames(
  file.path(cache_dir, sprintf("tas_gs_%d.tif", years)),
  years
)
...
# Sanity check — growing season Central Europe
cat("Value range (first year):",
    paste(round(as.numeric(global(ann_stack[[1]], "range", na.rm = TRUE)), 1),
          collapse = " – "), "°C\n")

#### Per-pixel linear trend (°C / year) ####

trend_r <- app(ann_stack, function(v) {
  if (anyNA(v)) return(NA_real_)
  n   <- length(v)
  x_c <- seq_len(n) - (n + 1L) / 2   # centred index, self-contained for parallel workers
...
# ------------------------------------------------------------------------------
# 3. REGION CLASSIFICATION ON THE RASTER GRID (DE excluded > AT > CZSK)
# ------------------------------------------------------------------------------
...
# TIME SERIES: Mean +/- SE growing-season temperature 
...
# SPATIAL MAP: Warming trend across the buffer
...
#add data veg. plots
dat_sf <- read_csv('data/model_data/contrast.based_data.csv', show_col_types = FALSE) %>%
  st_as_sf(coords=c('rsrv_lon', 'rsrv_lat'), crs='WGS84')
