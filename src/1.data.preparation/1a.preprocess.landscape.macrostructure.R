...
# new ids
buf_ids <- buf_shap.tot %>% st_drop_geometry() %>% mutate(buffer_id = 1:nrow(.))
buf_shap <- buf_shap %>% left_join(buf_ids) %>% select(-plot_id)
buf_shap.tot <- buf_shap.tot %>% left_join(buf_ids) %>% select(-plot_id)

# load resurvey data
plots_sel <- 'data/EVA&GRACE_header_20260202.csv' %>%
...
buf_shap_new <- buf_shap %>% 
  ...
  left_join(
    buf_new_assigned %>% st_drop_geometry() %>% filter(resurvey==0), by = 'buffer_id', relationship = 'many-to-many'
  ) %>%
  select(buffer_id, resurvey, plot_id, dist_min, everything()) 

# overwrite buffer shape file
buf_shap_new %>%
  write_rds('landscape/buffer_union_shape.rds', compress = 'gz') 



#5. rasterize landscapes ####

buf_shap_new <- read_rds('landscape/buffer_union_shape.rds')
...
for (i in names(raster_res)) {
  rast(raster_res[[i]]) %>%
    writeRaster(
      paste0('landscape/buffer_rasters/',i,'.tif'),
      overwrite = T
    )
}
Sys.time()-st # about 1 minute